...
import { imageColor } from "~/utils/color/color-fac";

export function ColorFac() {
  ...
  const getImageColor = async () => {
    ...
    setLoading(true);
    const color = await imageColor(
      document.getElementById("preview-img") as HTMLImageElement
    );
    console.log("fac color", color);
    setLoading(false);
    if (color) {
      setMainColor(color.hexa);
    }
  };
  ...
  useEffect(() => {
    ...
    return () => {
      console.log('fac: removeImageLoadedCallback')
